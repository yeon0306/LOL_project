...
| 포지션 | 위치 | 역할 | 설명 |
|-------|-----|----|-----|
|탑(TOP)|상단라인|라인교전,스플릿 푸시|탑 라인은 가장 윗라인을 말하며 개인의 실력이 가장 크게 영향을 받는 포지션이다.챔피언의 종류에 따라 1레벨 교전이나 3레벨 교전에서 승부가 결정되는 경우가 많으며 한 번 승부가 갈리면 역전하기 어려운 상황이 있다. 이러한 경우에는 정글의 도움을 받아 라인전에서 우세를 가져와야 하는데 탑은 정글의 도움을 받기가 가장 어렵다. 그러나 탑에서 우위를 점하는 경우 끊임없이 상대 탑라이너를 성장하지 못하게 하는 사이드운영에 유리하다. 반대로 팀이 지고 있는 경우에도 사이드 운영을 통해 변수를 창출하기 좋은 라인이다. | 
