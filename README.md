...
0825, 0915, 0917 각 날짜별 플래티넘 이상의 데이터와 전체 데이터의 승패예측 모델 정확도이다. 
종합적으로, 모델은 플래티넘 이상의 데이터는 높은 정확도를 보이며 전체 데이터는 약간 하락하는 경향을 보이지만 모두 정확도가 92% 이상으로, 이는 승패예측을 고려한 특성 데이터를 활용한 모델의 성능이 좋은 것을 알 수 있다.  
